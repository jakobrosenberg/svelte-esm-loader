...
import { getUnaliasedFilePath } from './utils/unalias.js'
import { getOptionsFromQuery } from './utils/misc.js'

export const svelteLoader = async (input) => {
    const options = {
        compilerOptions: {},
        svelteExtensions: [
            '\\.svelte',
    ...
    const { svelteExtensions, compilerOptions } = options
    ...
    const svelteExtRegex = new RegExp(`(${svelteExtensions.join('|')})(\\?.*)?$`)
    const svelteKitPathRegex = /\$app\//
    const assetExtsRegex = new RegExp(`(${assetExts.join('|')})$`)
    const allExtRegex = new RegExp(`(${[...svelteExtensions, ...assetExts].join('|')})(\\?.*)?$`)
    ...
    const _svelteLoader = {
        ...
        transform: async (source, opts) => {
            ...
            if (opts.url.match(svelteExtRegex)) {
                const fileOptions = getOptionsFromQuery(opts.url.split('?')[1])
    ...
    return createLoader(_svelteLoader)
}
